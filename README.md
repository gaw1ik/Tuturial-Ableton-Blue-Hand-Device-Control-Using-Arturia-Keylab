...
## Thoughts
You can control any device that is selected with the blue hand, although frankly I'm unsure as to how it chooses to map to devices with more than 8 parameters or how it chooses the order. It might just choose the first 8. Not sure. Regardless, I think the best way to work with this, is to make use of the macros feature in Live's instrument racks and audio effect racks. The macros in an audio effect rack, for instance, allow you to map 8 parameters to any parameter on any device in the rack. I think this is a really great way to organize the most important effect parameters on each track in your Live set. Furthermore, Live automatically places the blue hand on the first device in a track when that track is selected, so if you always place an instrument or audio effect rack <i>first</i> on a track and set up the macros, you can control 8 parameters on that track from your controller simply by selecting the track in Live, just like the track "record static" is selected in the screenshot below. 
...
Many controllers - including the KeyLab - have track selection control buttons, so this means you can potentially control all the effects and automation from your controller without ever having to touch the mouse or keyboard as long as you're willing to boil down every track in your set to no more than 8 parameters. I'm sure some people will find that overly limiting, but personally, I actually like having a little bit of limitation, so I find this quite appealing. Cool concept, although in practice the KeyLab Essential's track switching buttons stop functioning when I am in the User map mode putting a swift end to my hopes and dreams. They only seem to work in the DAW map mode, and while I cannot really think of a good reason why this would be the case, it does seem to be an unfixable issue. It's still possible to jump into DAW mode when you want to switch tracks, but in my opinion, that's just too many button presses for such an essential function (no Arturia pun intended). For now, I will still be selecting tracks with mouse or keyboard arrow keys :(
